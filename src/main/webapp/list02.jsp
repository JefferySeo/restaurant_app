...
<%@ include file="include/header.jsp" %>
		...
		%>
				<input type="hidden" id="lat" class="lat" name="lat" value="<%= rs.getFloat("lat") %>"/>
				<input type="hidden" id="lon" class="lon" name="lon" value="<%= rs.getFloat("lon") %>"/>
					<li>
        ...
        %>
				...
				<% if(pg > 1){ %>
			    	<a href="?sectordetail=<%=param %>&city=<%=param2 %>&cpage=<%=pg-1 %>" data-theme="c" data-role="button" data-icon="arrow-l" data-inline="true">이전</a>	    
			    <% } %>
			    <% if(pg < totalPages){ %>
			    	<a href="?sectordetail=<%=param %>&city=<%=param2 %>&cpage=<%=pg+1 %>" data-theme="c" data-role="button" data-icon="arrow-r" data-inline="true">다음</a>
				<% } %>
				</div>
            </div>
        </div>
